...
class Individual():
    def __init__(self,MASS,r_i,THRESHOLD):
        ...
        self.Teh_inactive = []
        self.Teh_total = []
        self.E_G = 0.97 #emmisitivity for soil
    ...
    def update_rb(self,temp_K,e_s,e_a):
        ...
        self.rb = ((specific_heat*air_density)/hc)/100
                    ...
                    if hour >= 20 or hour <= 5:
                        if temp[hour][row][column] > 5.0 and temp[hour][row][column] < 25.0:
                            if self.activity_status == 0.0: #0.0 means the salamander is active
                                ...
                                e_a_current = float(self.calculate_ea(dewpoint))
                                e_s_current = float(self.calculate_es(temp_current[hour][row][column] + 273.15))
                                rh_current = e_a_current/e_s_current
                                e_s_original = float(self.calculate_es(temp_K_original))
                                e_a_original = rh_current * e_s_original
                                e_a_new = e_a_original + (e_a_original*humidity_scenario)
                                vpd = (e_s_original-e_a_new)
                                self.elevm = elev[row][column]
                                self.update_rb(temp_K_original,e_s_original,e_a_new)
                                self.T_eh = self.calculate_Teh(self.Rs,self.rb,self.diameter,original_temp,self.elevm,vpd,self.Rabs(original_temp),self.radiative_conductance(original_temp))
                                if self.acclimation_status == 0.0: #no thermal sensitivity of resistance to water loss
                                    ...
                                    Rs = self.ri_acclimation(self.T_eh)
                                rho = (e_s_original/(temp_K_original*Rv))-(e_a_new/(temp_K_original*Rv)) #calculate water vapor density gradient
                                EWL = self.surface_area*(rho/(Rs+(self.rb*0.93))) #adjust boundary layer here for water vapor
                                self.EWL = EWL
                                hourly_loss = self.EWL*3600.0
                                if self.mass_to_lose > hourly_loss:
                                    self.mass_to_lose -= hourly_loss
                                    self.activity += 1.0
                                    energy_intake = ((((0.00383582 + (-0.00252254)*self.T_eh + 0.00090893*self.T_eh**2 + (-2.52723e-5)*self.T_eh**3)*1000)*self.mass)/24.)
                                    self.energy_status += energy_intake
                                    self.mass_to_lose += (energy_intake/22000.0)*2.33
                                    volume_oxygen = ((((10.0**((0.04618974*self.T_eh)+0.59925591*log10(self.mass)+0.86009768))/1000.0)*1.5)*20.1)
...
list_of_thresholds = [0.035,0.07,0.1] # [0.035,0.07,0.1]
list_of_humidities = [0.0,0.25,-0.25] #[0.0,0.25,-0.25]
list_of_depths = [20] #[5.0,20.0]
counter = 0.0
